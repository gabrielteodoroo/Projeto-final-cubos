...
export default function TableCobrancaStatus() {
  const [localArray, setLocalArray] = React.useState([])
  const { value, setShowModalEditarCobranca, setControl, setIdCobranca, control, statusCobranca } = useHooke()

  const [localArrayValor, setLocalArrayValor] = React.useState([])
  async function handlesCobranca() {
    try {
      const response = await api.get(`detalharcobrancas/${statusCobranca}`,
        {
          headers: {
      ...
      <Table aria-label="simple table">
        ...
        <TableBody>
          {localArrayValor.map((row, index) => (
            ...
            >
              <TableCell align="left">{row.nome_cliente}</TableCell>
